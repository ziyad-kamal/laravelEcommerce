...
class ItemsController extends Controller
{
    ...
    public function create(ItemRequest $request): RedirectResponse
    {
        try {
            ...
            $filter_data = $this->filter_req_items($request, $file_name);

            Items::create([
                'name'        => $filter_data['name'] ,
                'slug'        => SlugService::createSlug(Items::class, 'slug', $filter_data['name']),
                'description' => $filter_data['description'],
                'condition'   => $filter_data['condition'],
                'price'       => $filter_data['price'],
                'photo'       => $filter_data['file_name'],
                'date'        => now(),
    ...
    public function show(Request $request): View|RedirectResponse|JsonResponse
    {
        try {
            ...
            $selected_category = $request->has('selected_category') ? $request->get('selected_category') : 0;
    ...
    public function update(ItemRequest $request): JsonResponse
    {
        try {
            ...
            $item->name        = $filter_data['name'];
            $item->description = $filter_data['description'];
            $item->condition   = $filter_data['condition'];
            $item->price       = $filter_data['price'];
            $item->category_id = $request->get('category_id');
            $item->photo       = $filter_data['file_name'];
